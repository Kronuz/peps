...
* Globally enabled, either via a new ``-L`` flag to the Python interpreter or
  ...
  *all* new imports after the call immediately lazy.

* In module enabled, via ``importlib.enable_lazy_imports_in_module()``.
...
This flag does not reflect whether the current status of lazy imports is enabled
or disabled (as a result of using ``importlib.set_lazy_imports()`` or
``importlib.enable_lazy_imports_in_module()``). The ``-L`` flag is propagated
from to new Python subprocesses by using ``sys.flags.lazy_imports``, so in an
analogous way, new subprocesses don't necessarily reflect the current status of
lazy imports at the time the subprocess was spawned either.

Actual current status of whether lazy imports is enabled or not at any moment
can be retrieved using ``importlib.is_lazy_imports_enabled()``, which will
return ``True`` if lazy imports is enabled or ``False`` otherwise.
...
Anytime a key is looked up in a dictionary, as to extract any of its values, the
...
For authors of C extension modules, the proposed stable C API is as follows:

* ``importlib.set_lazy_imports()`` is available as
  ``PyImport_SetLazyImports(PyObject *names_or_callback_or_null)``.

* ``importlib.is_lazy_import`` is available as
  ``PyDict_IsLazyImport(PyObject *dict, PyObject *key)``.

* ``importlib.is_lazy_imports_enabled()`` is available as
  ``PyImport_IsLazyImportsEnabled()``.

* ``PyDict_ResolveLazyImports(PyObject *dict)`` resolves all lazy objects in a
