...
object immediately before returning it. To avoid having side effects mutating
dictionaries midway an iteration, all lazy objects in a dictionary are resolved
prior starting an iteration; this could incur a performance penalty when using
...
(`dk_lazy_imports` <https://github.com/python/cpython/blob/3.11/Include/internal/pycore_dict.h#L105>`_),
...
and such resolved value is returned from the lookup function. A dictionary could
...
resolved object. For this, the lazy import object contains a cached pointer to
the resolved object, to be returned immediately upon next access and the attempt
to replace the lazy import object with the resolved value will be re-tried.
